from src.constants import TWO_PI, SQRT_2
...
def solve_ellipse_semi_major_axis(ap: number, pe: number) -> float: #sMa
    rmax, rmin = max(ap, pe), min(ap, pe)
    return (rmax + rmin) / 2

def solve_ellipse_semi_minor_axis(ap: number, pe: number) -> float: #sma
    rmax, rmin = max(ap, pe), min(ap, pe)
...
def orbital_period(body: dict, ellipse_semi_major_axis: number) -> number:
    return TWO_PI * sqrt(ellipse_semi_major_axis / body['mu'])
