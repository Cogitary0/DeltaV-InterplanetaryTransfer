...
def type_orbit(energy_spacecraft: number) -> str:
    if energy_spacecraft < 0:
        return "elliptical orbit"

    elif energy_spacecraft == 0:
...
def time_transfer(body: dict, start_radius: number, end_radius: number) -> float:
    return PI * sqrt((start_radius + end_radius) ** 3 / 8 * body['mu'])


def angle_transfer(start_radius: number, end_radius: number) -> float:
    return PI * (1 - 0.353553391 * sqrt((start_radius / end_radius + 1) ** 3))
